...
module HexletCode
  class << self
    def form_for struct, url={}, &block
      ...
      if url.key?(:url)
        print "<form action='#{url.fetch(:url)}' method='post'>\n"
    ...
    def input param_name, **field_options
      ...
      @field_params = @params.each_with_object({}) do |(name, value), hash|
      ...
      is_textarea? param_name, **field_options

      puts @field.join
    end

    def is_textarea? param_name, **field_options
      @field = []

      return @field << '  no param' if !@params[param_name]

      case field_options[:as]
      when :text
        @field << '  <textarea '
        @field << @field_params.fetch(param_name)
        @field << '>'
